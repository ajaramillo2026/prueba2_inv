...
class ActividadController {
    ...
    // Método principal: Renderiza el panel unificado (KPIs, Carga de Trabajo, Filtros y Tabla)
    public function index() {
        // 1. Capturar filtros de tiempo. Si vienen vacíos, toman el mes y año actuales.
        ...
        // Capturar parámetros de paginación dinámica
        ...
        if ($_SESSION['usuario_tipo'] === 'basico') {
            ...
            // Administradores e Intermedios pueden usar el filtro libremente
        ...
        // 2. Obtener el universo total de registros filtrados para calcular la paginación matemática
        ...
        // Recortamos el arreglo para traer exclusivamente el bloque de la página solicitada
        $filtros['limit']  = $registros_por_pagina;
        $filtros['offset'] = $inicio_limit;
        $actividades = $this->modeloAct->obtenerFiltradas($filtros);
        ...
        foreach ($todasLasFiltradas as $a) {
            // Si el rol es básico y la actividad le pertenece a otro operador, se ignora en la suma
        ...
        // 4. Inicializar y procesar la matriz de Carga de Trabajo Mensual
        $carga_trabajo = [];
        foreach ($usuarios as $u) {
            $carga_trabajo[$u['id']] = [
                'nombre' => $u['nombre'], 'rol' => $u['tipo_usuario'],
                'pendiente' => 0, 'proceso' => 0, 'finalizado' => 0, 'total_activo' => 0
            ];
        }
        ...
        foreach ($todasLasFiltradas as $am) {
            if (!empty($am['usuario_id']) && isset($carga_trabajo[$am['usuario_id']])) {
                $st = $am['status'];
                if ($st == 'pendiente' || $st == 'proceso') {
                    $carga_trabajo[$am['usuario_id']][$st]++;
                    $carga_trabajo[$am['usuario_id']]['total_activo']++;
                }
                if ($st == 'finalizado') {
                    $carga_trabajo[$am['usuario_id']]['finalizado']++;
                }
            }
        }
        // Ordenación de mayor a menor volumen activo
        uasort($carga_trabajo, function($a, $b) { return $b['total_activo'] <=> $a['total_activo']; });
    ...
    // API Asíncrona (AJAX): Calcula y responde en tramas JSON los KPIs en tiempo real
    ...
    // Genera la descarga del archivo Excel respetando la segmentación activa de la barra
    public function exportarExcel() {
        ...
        // Forzar cabeceras del protocolo HTTP para descarga directa de ficheros binarios de Excel
        header("Content-Type: application/vnd.ms-excel; charset=utf-8");
        header("Content-Disposition: attachment; filename=Reporte_Actividades.xls");
        header("Pragma: no-cache");
        header("Expires: 0");

        // Imprimir la estructura interpretada nativamente por Microsoft Excel
        echo "<table border='1'>";
