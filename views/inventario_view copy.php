                ...
                <form method="GET" action="index.php" class="row g-2 align-items-end">
                    ...
                    <div class="col d-grid gap-1">
                        ...
                        <a href="index.php?accion=exportar_inventario&f_nombre=<?php echo urlencode($_GET['f_nombre'] ?? ''); ?>&f_bajo_stock=<?php echo urlencode($_GET['f_bajo_stock'] ?? ''); ?>" 
                           class="btn btn-outline-success btn-sm fw-bold shadow-sm" title="Descargar Excel"><i class="bi bi-file-earmark-excel"></i> Excel</a>
                    </div>
                </form>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Inicialización segura de DataTables local en español (Sintaxis jQuery)
    if (typeof $ !== 'undefined' && $.fn.DataTable) {
        $('#tablaInventario').DataTable({
            "pageLength": 10,
            "lengthMenu": [[5, 10, 25, 50, -1], [5, 10, 25, 50, "Todos"]],
            "order": [[2, "asc"]], // Prioriza mostrar arriba las unidades más escasas automáticamente
            "language": {
    ...
    document.body.addEventListener('click', function(event) {
        ...
        if (boton) {
            event.preventDefault(); // Detiene rebotes de hilos o comportamientos url por defecto
            ...
            // DISPARO Y RENDERIZACIÓN DE LA MODAL POR API NATIVA DE BOOTSTRAP 5 (CERO TIMEOUTS)
